   ...
   `
     ...
     `
        ...
        for (const kalem of magazaKalemleri || []) {
          ...
          const urunAdi = Array.isArray(kalem.urunler)
            ? kalem.urunler[0]?.urun_adi
            : kalem.urunler?.urun_adi;
...
`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Satış kaydedilirken hata oluştu: ${errorMessage}`,
        variant: "destructive",
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({
        title: t("common.error"),
        description: `Satış silinirken hata oluştu: ${errorMessage}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleEdit = async (satisDetay: SatisDetayViewRow) => {
    ...
    try {
      ...
      toast({
        title: t("common.error"),
        description: `Düzenleme sırasında hata oluştu: ${errorMessage}`,
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div>
              ...
              <div>
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="uyumlu">
                      {t("sales.compatible")}
                    </SelectItem>
                    <SelectItem value="uyumsuz">
                      {t("sales.incompatible")}
                    </SelectItem>
                    <SelectItem value="rehber_yok">
                      {t("sales.noGuideNotification")}
                    </SelectItem>
                    <SelectItem value="magaza_yok">
                      {t("sales.noStoreNotification")}
                    </SelectItem>
                  </SelectContent>
                </Select>
              </div>
        ...
        <DialogContent className="max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-lg sm:text-xl">
              {t("sales.commissionRates")}
            </DialogTitle>
            <DialogDescription className="text-sm">
              {t("sales.commissionRatesDescription")}
            </DialogDescription>
          </DialogHeader>
          {selectedSatis && (
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  <Label className="text-sm">
                    {t("sales.agencyCommissionAmount")}
                  </Label>
                  <Input
                    type="text"
                ...
                <div>
                  <Label className="text-sm">
                    {t("sales.captainCommissionAmount")}
                  </Label>
                  <Input
                    type="text"
                ...
                <div>
                  <Label className="text-sm">
                    {t("sales.officeCommissionAmount")}
                  </Label>
                  <Input
                    type="text"
          ...
          <form onSubmit={handleSubmit}>
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div>
                  <Label htmlFor="grup_gelis_tarihi">
                    {t("sales.groupArrivalDate")}
                  </Label>
                  <Input
                    type="date"
                ...
                <div>
                  <Label htmlFor="satis_tarihi">
                    {t("sales.storeEntryDate")}
                  </Label>
                  <Input
                    type="date"
              ...
              <div className="space-y-2">
                ...
                {satisUrunleri.map((urun, index) => (
                  ...
                  >
                    {/* Ürün seçimi - Sadece mağaza ürünlerini göster */}
                    <div className="sm:col-span-2 lg:col-span-1">
                      ...
                      <div className="flex items-center space-x-2">
                        <Select
                          ...
                          disabled={
                            !formData.magaza_id || magazaUrunleri.length === 0
                          }
                        >
                          <SelectTrigger className="text-sm">
                            <SelectValue
                              placeholder={
                                !formData.magaza_id
                                  ? t("sales.selectStoreFirst")
                                  : magazaUrunleri.length === 0
                                  ? t("sales.noProductsInStore")
                          ...
                          <SelectContent>
                            {magazaUrunleri.map((magazaUrun) => {
                              const urunAdi = magazaUrun.urunler?.urun_adi;

                              return (
                                <SelectItem
                                  key={magazaUrun.urun_id}
                                  value={magazaUrun.urun_id}
                                  className="text-sm"
                                >
                                  {urunAdi || "Ürün adı bulunamadı"}
                                </SelectItem>
                              );
                            })}
                          </SelectContent>
                      ...
                      {!formData.magaza_id && (
                        <p className="text-xs text-gray-500 mt-1">
                          {t("sales.selectStoreToSeeProducts")}
                        </p>
                      )}
                    </div>
      ...
      >
        <DialogContent className="w-[95vw] max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm">
              {t("sales.unsavedChangesDescription")}
            </DialogDescription>
          </DialogHeader>
      ...
      <AddUrunDialog
        ...
        selectedMagazaId={formData.magaza_id}
      />
      <AddMagazaDialog
