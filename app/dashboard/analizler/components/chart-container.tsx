...
import type { AnalysisData, AnalysisFilters, MetricType } from "../page";
...
export function ChartContainer({
  ...
  const getSortValue = (item: AnalysisData, metricType: MetricType) => {
    if (metricType === "totalSales") return item.totalSales;
    if (metricType === "totalAmount") return item.totalAmount;
    if (metricType === "paxAverage") return item.paxAverage;
    return 0;
  };
  ...
  const chartData = data
    ...
    .slice(0, 10) // En iyi 10 öğe
    .map((item) => ({
      name:
        item.name.length > 15 ? item.name.substring(0, 15) + "..." : item.name,
      fullName: item.name,
      satisAdedi: item.totalSales, // This is now total pax if 'adet' is pax
      toplamTutar: item.totalAmount,
      islemSayisi: item.salesCount,
      ortalamaTutar: item.averageAmount,
      paxOrtalamasi: item.paxAverage, // New: pax average
    }));
    ...
    const formatter = (value: any, name: string) => {
      if (typeof value === "number") {
        if (filters.metricType === "totalAmount") {
          ...
          return [value.toFixed(2), mainMetricLabel];
        }
        ...
        return (
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={chartData}>
              ...
              />
              {/* Optionally show total amount for admin if not the main metric */}
    ...
    <Card>
      <CardHeader>
        <CardTitle>
          ...
          {filters.analysisType === "operator" && "Operatör"} Performans Analizi
        </CardTitle>
        <CardDescription>
