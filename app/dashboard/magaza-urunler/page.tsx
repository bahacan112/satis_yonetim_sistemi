...
import { Edit, Trash2, Plus, Store, Package } from "lucide-react"
...
export default function MagazaUrunlerPage() {
  ...
  const [editingMagazaUrun, setEditingMagazaUrun] = useState<MagazaUrun | null>(null)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingMagazaUrun) {
        const { error } = await supabase.from("magaza_urunler").update(dataToSave).eq("id", editingMagazaUrun.id)
        ...
        const { error } = await supabase.from("magaza_urunler").insert(dataToSave)
        if (error) throw error
  ...
  const handleEdit = (magazaUrun: MagazaUrun) => {
    ...
    const magaza = magazalar.find((m) => m.id?.toString() === magazaUrun.magaza_id?.toString())
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!confirm("Bu mağaza ürünü kaydını silmek istediğinizden emin misiniz?")) return

    try {
      const { error } = await supabase.from("magaza_urunler").delete().eq("id", id)
      if (error) throw error
  ...
  const toggleAktif = async (id: number, currentAktif: boolean) => {
    try {
      const { error } = await supabase.from("magaza_urunler").update({ aktif: !currentAktif }).eq("id", id)
    ...
    <div>
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Mağazaların sattığı ürünleri ve komisyon oranlarını yönetin</p>
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Mağaza Ürün Listesi ({magazaUrunler.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {magazaUrunler.map((magazaUrun) => (
                  <TableRow key={magazaUrun.id}>
                    <TableCell>{magazaUrun.id}</TableCell>
                    <TableCell>{magazaUrun.magazalar?.firmalar?.firma_adi || "-"}</TableCell>
                    <TableCell className="font-medium">{magazaUrun.magazalar?.magaza_adi || "-"}</TableCell>
                    <TableCell>{magazaUrun.urunler?.urun_adi || "-"}</TableCell>
                    {userRole === "admin" && <TableCell>%{magazaUrun.acente_komisyonu}</TableCell>}
                    {userRole === "admin" && <TableCell>%{magazaUrun.rehber_komisyonu}</TableCell>}
                    {userRole === "admin" && <TableCell>%{magazaUrun.kaptan_komisyonu}</TableCell>}
                    {userRole === "admin" && <TableCell>%{magazaUrun.ofis_komisyonu}</TableCell>}
                    <TableCell>
                      <Badge
                        variant={magazaUrun.aktif ? "default" : "secondary"}
                        className={`cursor-pointer ${
                          magazaUrun.aktif ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"
                        }`}
                        onClick={() => toggleAktif(magazaUrun.id, magazaUrun.aktif)}
                      >
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={() => handleEdit(magazaUrun)}>
                          <Edit className="w-4 h-4" />
                        </Button>
                        {userRole === "admin" && (
                          <Button variant="destructive" size="sm" onClick={() => handleDelete(magazaUrun.id)}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingMagazaUrun ? "Mağaza Ürünü Düzenle" : "Yeni Mağaza Ürünü Ekle"}</DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) => setFormData({ ...formData, magaza_id: value })}
                required
              ...
              <>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, acente_komisyonu: e.target.value })}
                    required
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, rehber_komisyonu: e.target.value })}
                    required
                    placeholder="Örn: 10"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, kaptan_komisyonu: e.target.value })}
                    required
                    placeholder="Örn: 5"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, ofis_komisyonu: e.target.value })}
                    required
                    placeholder="Örn: 2.5"
                  />
                </div>
            ...
            <div className="flex items-center space-x-2">
              <Checkbox
                ...
                onCheckedChange={(checked) => setFormData({ ...formData, aktif: checked as boolean })}
              />
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit">{editingMagazaUrun ? "Güncelle" : "Ekle"}</Button>
            </div>
          </form>
        </DialogContent>
      </Dialog>
