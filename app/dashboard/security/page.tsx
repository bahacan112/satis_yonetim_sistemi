...
export default function SecurityPage() {
  ...
  const handleDeleteLogs = async (deleteAll = false) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Log silme işlemi başarısız")
    } finally {
  ...
  const UserCell = ({ log }: { log: LogEntryWithUserInfo }) => {
    ...
    return (
      <div className="flex items-center gap-2">
        <Avatar className="h-6 w-6">
          <AvatarFallback className="text-xs">{getUserInitials(log.user_name)}</AvatarFallback>
        </Avatar>
        <div className="flex flex-col">
          <span className="text-sm font-medium">{log.user_name}</span>
          {log.user_role && (
            <Badge className={`text-xs ${getRoleBadgeColor(log.user_role)} h-4`}>{log.user_role}</Badge>
          )}
        </div>
      </div>
    )
    ...
    return (
      <div className="container mx-auto p-6">
        <Alert>
          ...
          <AlertDescription>Bu sayfaya erişim yetkiniz bulunmamaktadır.</AlertDescription>
        </Alert>
      </div>
    )
  }
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-2">Sistem aktivitelerini ve güvenlik olaylarını izleyin</p>
        </div>
            ...
            <DialogContent className="sm:max-w-md">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    className="mt-1"
                  />
                  <p className="text-xs text-gray-500 mt-1">Bu tarihten önceki tüm loglar silinecek</p>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2">
                    {getQuickDateOptions().map((option) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        onClick={() => setSelectedCutoffDate(option.value.slice(0, 16))}
                        className="text-xs"
                      >
                        {option.label}
                      </Button>
                ...
                {statistics && (
                  <div className="bg-gray-50 p-3 rounded-lg text-sm">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="font-medium">Mevcut Log İstatistikleri</span>
                    </div>
                    <div className="space-y-1 text-xs text-gray-600">
                      <div>Toplam kayıt: {statistics.total_requests}</div>
                      {statistics.oldest_log && (
                        <div>
                          En eski: {format(new Date(statistics.oldest_log), "dd/MM/yyyy HH:mm", { locale: tr })}
                        </div>
                      )}
                      {statistics.total_size_mb && <div>Boyut: {statistics.total_size_mb} MB</div>}
                    </div>
                  </div>
                )}
              </div>
              ...
              <DialogFooter className="flex-col sm:flex-col gap-2">
                <Button
                  ...
                  className="w-full"
                >
                  {deleteLoading ? "Siliniyor..." : "Seçilen Tarihten Önceki Logları Sil"}
                </Button>
                <Button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">{statistics.successful_requests} başarılı</p>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{formatDuration(statistics.avg_response_time)}</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">En Aktif Kullanıcı</CardTitle>
              <User className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-sm font-bold truncate" title={statistics.most_active_user}>
                {statistics.most_active_user || "Bilinmiyor"}
              </div>
              <p className="text-xs text-muted-foreground">En çok işlem yapan</p>
            </CardContent>
          </Card>
        </div>
      )}
        ...
        <TabsContent value="logs" className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Son {filteredLogs.length} kayıt gösteriliyor</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {filteredLogs.map((log) => (
                      <TableRow key={log.id}>
                        ...
                        <TableCell>
                          <Badge className={getMethodBadgeColor(log.method)}>{log.method}</Badge>
                        </TableCell>
                        <TableCell>
                          {log.success ? (
        ...
        <TabsContent value="errors" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Son {errorLogs.length} hata kaydı</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <Table>
                ...
                <div className="space-y-2">
                  {statistics?.most_active_table && (
                    <div className="flex items-center justify-between p-2 bg-blue-50 rounded">
                      <span className="font-medium">{statistics.most_active_table}</span>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600 mt-2">Toplam isteklerin hata oranı</p>
                </div>
              </CardContent>
            </Card>
              ...
              <Card className="md:col-span-2">
                ...
                <CardContent>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {statistics.oldest_log && (
                      <div>
                        <Label className="text-sm text-gray-600">En Eski Log</Label>
                        <p className="font-mono text-sm">
                          {format(new Date(statistics.oldest_log), "dd/MM/yyyy HH:mm", { locale: tr })}
                        </p>
                      </div>
                    )}
                    {statistics.newest_log && (
                      <div>
                        <Label className="text-sm text-gray-600">En Yeni Log</Label>
                        <p className="font-mono text-sm">
                          {format(new Date(statistics.newest_log), "dd/MM/yyyy HH:mm", { locale: tr })}
                        </p>
                      </div>
                    )}
                    {statistics.total_size_mb && (
                      <div>
                        <Label className="text-sm text-gray-600">Toplam Boyut</Label>
                        <p className="font-mono text-sm">{statistics.total_size_mb} MB</p>
                      </div>
                    )}
                    <div>
                      <Label className="text-sm text-gray-600">Toplam Kayıt</Label>
                      <p className="font-mono text-sm">{statistics.total_requests}</p>
                    </div>
                  </div>
                </CardContent>
              </Card>
