...
CREATE OR OR REPLACE VIEW public.satislar_detay_view AS
SELECT
    s.id AS satis_id,
...
LEFT JOIN
    public.magaza_urunler mu ON sk.urun_id = mu.id AND s.magaza_id = mu.magaza_id -- CORRECTED LINE: Changed sk.magaza_id to s.magaza_id
LEFT JOIN
