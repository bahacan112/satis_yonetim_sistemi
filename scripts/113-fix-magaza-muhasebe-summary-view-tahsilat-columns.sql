...
-- satislar_detay_view'i yeni status sütunu ile yeniden oluştur (önceki başarılı versiyon)
...
-- magaza_satis_detaylari_view'i yeniden oluştur (önceki başarılı versiyon)
...
-- magaza_muhasebe_summary_view'i tahsilat payları ile yeniden oluştur
...
tahsilat_toplamlari AS (
    SELECT
        t.magaza_id,
        SUM(t.acente_payi) AS toplam_acente_tahsilat, -- acente_payi kullanıldı
        SUM(t.ofis_payi) AS toplam_ofis_tahsilat    -- ofis_payi kullanıldı
    FROM
        tahsilatlar t
    GROUP BY
        t.magaza_id
...
SELECT
    ...
    (COALESCE(st.toplam_acente_komisyon, 0) - COALESCE(tt.toplam_acente_tahsilat, 0)) AS kalan_acente_alacagi,
    (COALESCE(st.toplam_ofis_komisyon, 0) - COALESCE(tt.toplam_ofis_tahsilat, 0)) AS kalan_ofis_alacagi
FROM
    magazalar m
