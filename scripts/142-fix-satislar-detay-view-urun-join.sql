-- Satislar detay view'ındaki ürün join'ini düzelt

-- Önce mevcut view'ı düşür
DROP VIEW IF EXISTS public.satislar_detay_view CASCADE;

-- View'ı yeniden oluştur - doğru ürün join'i ile
...
SELECT
    ...
    s.operator_id,
    o.operator_adi,
    -- Ürün bilgileri - doğrudan urunler tablosundan
    msk.urun_id,
    u.urun_adi,
    msk.adet,
    msk.birim_fiyat,
...
LEFT JOIN
    public.urunler u ON msk.urun_id = u.id  -- Doğrudan ürün join'i
LEFT JOIN
...
-- View'a yorum ekle
COMMENT ON VIEW public.satislar_detay_view IS 'Satış detayları view - doğrudan ürün join ile düzeltildi';
