-- scripts/98-fix-satislar-detay-view-joins.sql
...
-- Step 2: Recreate satislar_detay_view with INNER JOINs for sales items and COALESCE for totals
...
SELECT
    ...
    msk.ofis_komisyonu,
    COALESCE(msk.adet * msk.birim_fiyat, 0) AS toplam_tutar, -- COALESCE eklendi
...
FROM
    public.satislar s
INNER JOIN public.magaza_satis_kalemleri msk ON s.id = msk.satis_id -- INNER JOIN
LEFT JOIN public.turlar t ON s.tur_id = t.id
LEFT JOIN public.operatorler o ON s.operator_id::uuid = o.id
...
SELECT
    ...
    COALESCE(rsk.adet * rsk.birim_fiyat, 0) AS toplam_tutar, -- COALESCE eklendi
...
FROM
    public.satislar s
INNER JOIN public.rehber_satis_kalemleri rsk ON s.id = rsk.satis_id -- INNER JOIN
LEFT JOIN public.turlar t ON s.tur_id = t.id
LEFT JOIN public.operatorler o ON s.operator_id::uuid = o.id
LEFT JOIN public.rehberler r ON s.rehber_id = r.id
LEFT JOIN public.magazalar m ON s.magaza_id = m.id
