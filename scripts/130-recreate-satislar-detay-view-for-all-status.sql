-- scripts/130-recreate-satislar-detay-view-for-all-status.sql
-- Recreate satislar_detay_view to show all status types including iptal and beklemede

-- Drop the existing view (if it still exists, though 129 should handle it)
DROP VIEW IF EXISTS public.satislar_detay_view;

-- Recreate satislar_detay_view with proper UNION to include both magaza and rehber items
...
SELECT
    ...
    rsk.adet,
    rsk.birim_fiyat,
    0 AS acente_komisyonu, -- Rehber items don't have commission rates
    ...
    rsk.status,
    0 AS acente_komisyon_tutari, -- Rehber items don't have commission amounts
