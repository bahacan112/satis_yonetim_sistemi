-- Satislar detay view'ını operatör join'i ile düzelt

-- Önce mevcut view'ı düşür
DROP VIEW IF EXISTS public.satislar_detay_view CASCADE;

-- View'ı yeniden oluştur - operatör join'i dahil
...
SELECT
    ...
    t.tur_adi,
    t.operator_id,
    o.operator_adi, -- Operatör adını turlar üzerinden al
    s.rehber_id,
    r.rehber_adi,
...
LEFT JOIN
    public.operatorler o ON t.operator_id = o.id -- Operatör join'i eklendi
LEFT JOIN
...
-- View'a yorum ekle
COMMENT ON VIEW public.satislar_detay_view IS 'Satış detayları view - mağaza ve rehber satış kalemlerini birleştirir, operatör bilgisi dahil';
