-- scripts/106-fix-magaza-muhasebe-group-by.sql
-- Fix GROUP BY clause in magaza_muhasebe_summary_view and recreate all views
...
-- Step 2: Ensure satislar.operator_id is UUID type
DO $$
DECLARE
    col_type TEXT;
    fk_name TEXT;
...
-- Step 3: Create satislar_detay_view
...
-- Step 4: Create magaza_satis_detaylari_view
...
-- Step 5: Create magaza_muhasebe_summary_view with corrected GROUP BY
...
GROUP BY
    m.id, -- Primary key of magazalar, functionally determines m.magaza_adi and m.firma_id
    m.magaza_adi, -- Selected non-aggregated column
    tf.total_tahsilat_for_firm; -- Value from subquery, treated as non-aggregated in outer select
