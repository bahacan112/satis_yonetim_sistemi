...
SELECT
    ...
    m.firma_id,
    f.firma_adi,
    COALESCE(SUM(msdv.toplam_tutar), 0) AS toplam_satis_tutari,
    COALESCE(SUM(msdv.acente_komisyon_tutari), 0) AS toplam_acente_komisyonu,
    COALESCE(SUM(msdv.rehber_komisyon_tutari), 0) AS toplam_rehber_komisyonu,
    COALESCE(SUM(msdv.kaptan_komisyon_tutari), 0) AS toplam_kaptan_komisyonu,
    COALESCE(SUM(msdv.ofis_komisyon_tutari), 0) AS toplam_ofis_komisyonu,
    COALESCE(SUM(t.acente_payi + t.ofis_payi), 0) AS toplam_tahsilat,
    (COALESCE(SUM(msdv.toplam_tutar), 0) - COALESCE(SUM(t.acente_payi + t.ofis_payi), 0)) AS kalan_bakiye
FROM
    public.magazalar m
LEFT JOIN
