-- scripts/117-update-muhasebe-summary-view-no-rehber-total.sql
-- Update magaza_muhasebe_summary_view to remove 'genel_toplam_satis'
-- and ensure only 'magaza' type sales contribute to store's accounting totals.
...
WITH sales_summary AS (
    ...
    SELECT
        ...
        -- Total sales for 'rehber' type items with 'onaylandı' status (for informational display if needed)
        ...
        -- Commission totals (only from 'magaza' type items with 'onaylandı' status)
