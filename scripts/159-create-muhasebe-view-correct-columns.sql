-- Doğru sütun adlarıyla muhasebe view'ını oluştur
...
WITH magaza_satis_ozet AS (
  SELECT 
    ...
    COALESCE(SUM(CASE WHEN msk.status = 'onaylandi' THEN msk.acente_komisyonu ELSE 0 END), 0) as toplam_komisyon_tutari,
    ...
    COALESCE(SUM(CASE WHEN msk.status = 'beklemede' THEN msk.acente_komisyonu ELSE 0 END), 0) as bekleyen_komisyon_tutari,
  ...
  GROUP BY m.id, m.magaza_adi, f.firma_adi
),
tahsilat_ozet AS (
  SELECT 
    t.magaza_id,
