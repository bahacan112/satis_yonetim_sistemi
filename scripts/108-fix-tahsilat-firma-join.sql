-- scripts/108-fix-tahsilat-firma-join.sql
-- Recreate all views, fixing the join logic for tahsilatlar to get firma_id from satislar.
...
tahsilat_summary AS (
    -- **FIXED LOGIC**: Aggregate tahsilat data by joining with satislar to get firma_id
    SELECT
        s.firma_id,
        SUM(t.acente_payi + t.ofis_payi) AS total_tahsilat_for_firm
    FROM public.tahsilatlar t
    JOIN public.satislar s ON t.satis_id = s.id -- Correctly join to find the firma
    WHERE s.firma_id IS NOT NULL
    GROUP BY s.firma_id
)
